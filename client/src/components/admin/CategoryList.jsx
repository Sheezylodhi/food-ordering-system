    ...
    <div className="p-4 sm:p-8 bg-slate-50 min-h-screen">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-500 font-medium text-sm sm:text-base">Manage your restaurant menu segments.</p>
        </div>
        ...
        {deleteId && (
          <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white p-6 sm:p-8 rounded-3xl w-full max-w-sm text-center shadow-2xl">
              ...
              <p className="text-slate-500 text-sm mb-6">Are you sure? This will remove all items associated with this category.</p>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left min-w-[400px]">
                  <thead className="bg-slate-50/50">
                    <tr>
                      <th className="p-4 sm:p-6 text-xs font-black text-slate-400 uppercase tracking-widest">Icon</th>
                      <th className="p-4 sm:p-6 text-xs font-black text-slate-400 uppercase tracking-widest">Name</th>
                      <th className="p-4 sm:p-6 text-xs font-black text-slate-400 uppercase tracking-widest text-right">Actions</th>
                    </tr>
                  </thead>
                    ...
                    {categories.map((cat) => (
                      <tr key={cat._id} className="hover:bg-indigo-50/30 transition-all">
                        <td className="p-4 sm:p-6">
                          <img src={cat.icon} className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl object-cover shadow-sm" alt={cat.name} />
                        </td>
                        <td className="p-4 sm:p-6 font-bold text-slate-800 text-sm sm:text-base">{cat.name}</td>
                        <td className="p-4 sm:p-6 text-right space-x-2">
                          <button onClick={() => setEditingCat(cat)} className="p-2 sm:p-3 bg-slate-100 text-slate-600 rounded-2xl hover:bg-indigo-600 hover:text-white transition-all"><Edit2 size={18} /></button>
                          <button onClick={() => setDeleteId(cat._id)} className="p-2 sm:p-3 bg-slate-100 text-red-500 rounded-2xl hover:bg-red-600 hover:text-white transition-all"><Trash2 size={18} /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
