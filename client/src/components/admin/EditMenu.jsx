...
import { Loader2, Tag, CheckCircle2, AlertCircle, LayoutGrid, Upload } from 'lucide-react';
...
const EditMenu = ({ item, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus({ type: 'error', message: 'Failed to update. Check connection.' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 block flex items-center gap-2">
              <LayoutGrid size={18}/> Product Visuals
            </label>
            <label className="relative flex flex-col items-center justify-center w-full h-64 sm:h-80 border-4 border-dashed border-indigo-100 rounded-3xl cursor-pointer bg-slate-50 hover:border-indigo-400 transition-all overflow-hidden">
        ...
        <div className="lg:col-span-4">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 space-y-6 lg:sticky lg:top-8">
            ...
            <button type="submit" disabled={loading} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold text-lg hover:bg-indigo-700 transition-all">
